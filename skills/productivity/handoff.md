...
When the user types `/handoff [focus?]`, compact the current conversation into a handoff document so another agent or session can continue the work.
...
3. **Adapt to focus** — if the user provided an argument (e.g. `/handoff write the tests`), tailor the "Pending work" and "Suggested skills" sections toward that goal.
...
- Keep the document under 400 lines — if context is larger, summarize aggressively
- After writing, print the file path so the user can find it
